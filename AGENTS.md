# Authoring & maintaining the `rpa-bugfix` skill
...
├─ SKILL.md              # COPY of skills/rpa-bugfix/SKILL.md (display / direct-copy install)
...
- `skills/rpa-bugfix/SKILL.md` — **canonical**. What the Claude Code / Codex plugin loaders read, and what you
  copy when installing into `~/.claude/skills/rpa-bugfix/`, `~/.cursor/skills/rpa-bugfix/`, etc.
...
```bash
cp skills/rpa-bugfix/SKILL.md SKILL.md
diff skills/rpa-bugfix/SKILL.md SKILL.md
```

Bundled assets (`references/`, `scripts/`, …) live **only** under `skills/rpa-bugfix/`, next to the canonical
...
- `skills/rpa-bugfix/SKILL.md` frontmatter — and the root `SKILL.md` copy.
...
- `name` **must** equal the directory name `rpa-bugfix` and the slash command (`/rpa-bugfix`).
...
Put scripts under `skills/rpa-bugfix/scripts/`. Add tests where it makes sense and make sure they pass before
committing.

## Commit checklist

- [ ] `skills/rpa-bugfix/SKILL.md` edited where needed.
- [ ] Root `SKILL.md` synced — `diff skills/rpa-bugfix/SKILL.md SKILL.md` is clean.
...
- [ ] README updated.
- [ ] Conventional commit message, e.g. `feat(rpa-bugfix): …` / `fix(rpa-bugfix): …`.
