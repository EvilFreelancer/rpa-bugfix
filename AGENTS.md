...
- `SKILL.md` (repo root) — **canonical**. The repo itself is the skill folder: copy or symlink the whole
  repository into `~/.claude/skills/rpa-bugfix/`, `~/.cursor/skills/rpa-bugfix/`, etc.
...
1. `.claude-plugin/marketplace.json` (catalog) - in the `rpa-bugfix` entry set `version` to the new number and
...
```bash
grep -A3 '"name": "rpa-bugfix"' .claude-plugin/marketplace.json
